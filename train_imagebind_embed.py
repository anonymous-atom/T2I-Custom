...
from ldm.modules.encoders.adapter import VisCtrl
from ldm.util import instantiate_from_config
# from ldm.modules.extra_condition.model_edge import pidinet
from ldm.modules.extra_condition.imagebind.models import imagebind_model
from ldm.modules.extra_condition.magebind.models.imagebind_model import ModalityType
...
if __name__ == '__main__':
    ...
    # ImageBind model
    net_G = imagebind_model.imagebind_huge(pretrained=True)
    net_G.eval()
    net_G.to(device)

    # Hook to get the features from ImageBind
    outputs = []
    def hook_fn(module, input, output):
        layer_name = module.__class__.__name__
        outputs.append(output)

    for layer_n in net_G.modality_trunks.vision.blocks:
        layer_n.norm_2.register_forward_hook(hook_fn)
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        for _, data in enumerate(train_dataloader):
            ...
            with torch.no_grad():
                ...
                inputs = { ModalityType.VISION: data['im'].cuda(non_blocking=True), }

                edge = net_G(inputs)

                edge = edge[ModalityType.VISION].cpu().numpy()

                g_device = outputs[0].device  # Assuming outputs is a list of tensors
                sum_activations = torch.zeros(16, 16, 1280).to(g_device)

                for actvn in outputs:
                    activations_without_cls = actvn[1:, :, :]
                    reshaped_activations = activations_without_cls.view(16, 16, 1280)

                    # Accumulate the activations
                    sum_activations += reshaped_activations
