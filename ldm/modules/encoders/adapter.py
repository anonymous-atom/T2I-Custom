...
class ResnetBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.block1 = nn.Conv2d(in_channels, out_channels, kernel_size=3, stride=1, padding=1)
        self.act = nn.ReLU()
        self.block2 = nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1)
...
class AdapterBlock(nn.Module):
    def __init__(self, in_channels, out_channels, downsample=False, in_conv = False):
        ...
        if self.in_conv:
            self.in_conv = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1)

        self.resnets = nn.Sequential(
            ResnetBlock(out_channels, out_channels),
            ResnetBlock(out_channels, out_channels)
        )
        self.zero_conv = zero_module(self, nn.Conv2d(out_channels, out_channels, kernel_size=1, stride=1))
    ...
    def forward(self, x):
        ...
        if self.in_conv:
            x = self.in_conv(x)
        x = self.resnets(x)
        zero_feature = self.zero_conv(x)
        return x, zero_feature

class AllBlocks(nn.Module):

    def __init__(self):
        super(AllBlocks, self).__init__()
        self.unshuffle = PixelUnshuffle(downscale_factor=8)
        self.conv_in = nn.Conv2d(81920, 320, kernel_size=3, stride=1, padding=1)
        self.body = nn.ModuleList([
            AdapterBlock(320, 320),
            AdapterBlock(320, 640, in_conv=True),
            AdapterBlock(640, 1280, downsample=True, in_conv=True),
            AdapterBlock(1280, 1280)
        ])
    ...
    def forward(self, x):
        ...
        for block in self.body:
            x, zero_feature = block(x)
            feature_list.append(zero_feature)
        return x, feature_list

class VisCtrl(nn.Module):
    def __init__(self):
        super(VisCtrl, self).__init__()
        self.adapter = AllBlocks()

    def forward(self, x):
        x, zero_feature_list = self.adapter(x)
        return x, zero_feature_list
